...
const getIronList = async(req,res) =>{
    let iron = new Set()
    try{
        let ironList = await Iron.find()
        for (i of ironList){
            iron.add(i.name)
        }
    }
...
const getIronStorageNonAdmin = async(req, res) => {
        ...
        for (const ironName in ironMap) {
            ...
            for (const radius in radii) {
                ...
                for (let i = 0 ; i < items.length ; i++) {                      
                    ...
                    rowitem["name"] = ironName
                    if ("weight" in rowitem){
                        rowitem["weight"] += items[i].weight
                    }
                    else{
                        rowitem["weight"] = items[i].weight
                    }
                    rowitem["radius"] = radius
                    rowitem["price"] =+ items[i].unitCost.toLocaleString()
                    rowitem["totalPrice"] += (parseFloat(items[i].weight/1000) * items[i].unitCost)
                }
                ironStorage.push(rowitem);
            }
        }
        console.log("ironStorage",ironStorage)
